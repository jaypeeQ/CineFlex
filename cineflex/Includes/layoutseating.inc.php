...
if(!isset($_SESSION['userid'])) {
    ...
    $_SESSION['loginfirst'] = 'Please Login first before booking a ticket.';
}
        ...
        foreach ($letters2 as $letter => $value) {
            ...
            while ($result = $sthshow2->fetch(PDO::FETCH_ASSOC)) {
                ...
                if ($result['FkTimeSlotId']) {
                    echo '<td style="background-color: red; font-size: 14px;max-height: 12px; max-width: 12px"> <input type="checkbox" disabled> ' . 'Stoel '. $result['SeatRow'] .' - '. $result['SeatNumber'] . '</td>';
